(define (domain warehouse)
  ...
  (:predicates 
      ...
      (requires-type ?c - command ?t - product-type)
)
...
(:action finish-command
      ...
      :precondition (and (working-on ?w ?c) 
                         (forall (?t - product-type) 
                             (imply (requires-type ?c ?t) (checkpoint-verified ?t ?c))))
      :effect (and 
          (free ?w) 
          (not (working-on ?w ?c)) 
          (command-complete ?c)
          ;;(not (mission-started ?c))
          (forall (?t - product-type) (not (type-scanned ?t ?c)))
          (forall (?t - product-type) (not (checkpoint-verified ?t ?c)))
      )
  )
)